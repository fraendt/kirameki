...
class KiramekiHelper {
    ...
    /**
     * Get a strain graph file buffer by its identifier
     * @param {string} identifier File identifier generated by uniqid
     * @returns {Promise<Buffer>} A promise resolving with the file buffer, rejecting on error
     */
    getStrainGraphBuffer(identifier) {
        return new Promise((resolve, reject) => {
            fs.readFile(`${__dirname}/../straingraphs/${identifier}`, (error, data) => {
                if (error) {
                    reject(error);
                }
    ...
    /**
     * Caches a Beatmap strain graph in the database and saves the graph as an image on disk for later user
     * @async
     * @param {object} database A database connection object
     * @param {string|number} beatmapID The beatmap ID of which the strain graph stems from 
     * @param {string} beatmapTitle The beatmap's title 
     * @param {string|number} strainMods A bitwise representation of the map modifiers applied to the strain graph 
     * @param {Buffer} graph A buffer containing the strain graph
     */
    cacheStrainGraph(database, beatmapID, beatmapTitle, strainMods, graph) {
        const identifier = uniqid();
        const graphDirectory = `${__dirname}/../straingraphs/${identifier}.png`;
        
        fs.writeFile(graphDirectory, graph, async (error) => {
            if (error) {
                return this.log(this.LogLevel.ERROR, 'STRAIN GRAPH CACHING ERROR', error);
            }

            await this.preparedQuery(
                database, 
                'INSERT INTO osu_strain_graphs (id, beatmap_id, beatmap_title, mods, graph) VALUES (NULL, ?, ?, ?, ?);',
                [beatmapID, beatmapTitle, strainMods, `${identifier}.png`]
            );
        });
    }
